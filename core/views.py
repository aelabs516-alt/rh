...
def home(request):
    ...
    if rol == 'COLABORADOR':
        try:
            ...
            total_colabs = 1
            registros = RegistroAsistencia.objects.filter(colaborador=colab)
        ...
        vacaciones_admin_json = json.dumps(vacaciones_admin_data)
    ...
    # Preparar actas
    actas_data = []
    for a in ActaDisciplinaria.objects.all():
        actas_data.append({
            'id': a.id,
    ...
    context = {
        ...
        'vacaciones_admin_json': locals().get('vacaciones_admin_json', '[]'),
        'registros_json': registros_json,
        'colaboradores_json': locals().get('colaboradores_json', '[]'),
